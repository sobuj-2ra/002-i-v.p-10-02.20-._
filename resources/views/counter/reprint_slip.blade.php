...
@section('page-content')
    ...
    <div id="printableArea" style="display: none;">
        <?php for ($i = 1; $i <= $slip->slip_copy; $i++){  ?>
        <table border=0 width="100%" class="tclass" style="padding: 0">
            ...
            <tr>
                <td colspan=2 style='margin: 105px 0'>
                    ...
                    <strong>Total:</strong> {{ $appServed->proc_fee+$appServed->sp_fee+$appServed->corrFee}}  <strong>Payment:</strong> {{ $appServed->Pmethod}} <br/>
    ...
    <script>
        ...
        $(window).on('afterprint', function () {
            window.location.href = "{{ url('/regular-passport-reprint') }}";
            // window.close();
        });
        <?php } ?>
    </script>
